...
bool MCP23S17::begin()
{
    ...
    if (!writeReg(MCP23S17_IOCR, MCP23S17_IOCR_SEQOP))
        return false;

    // Force INPUT_PULLUP
    if (!writeReg(MCP23S17_PUR_A, 0xFF))
        return false; //  0xFF == all UP
    if (!writeReg(MCP23S17_PUR_B, 0xFF))
        return false; //  0xFF == all UP
    return true;
}
...
bool MCP23S17::pinMode8(uint8_t port, uint8_t value)
{
    ...
    if (port == 0)
        writeReg(MCP23S17_DDR_A, value);
    if (port == 1)
        writeReg(MCP23S17_DDR_B, value);
    _error = MCP23S17_OK;
    return true;
}
...
bool MCP23S17::write8(uint8_t port, uint8_t value) // port = 0..1
{
    ...
    if (port == 0)
        writeReg(MCP23S17_GPIO_A, value);
    if (port == 1)
        writeReg(MCP23S17_GPIO_B, value);
    _error = MCP23S17_OK;
    return true;
}
...
int MCP23S17::read8(uint8_t port)
{
    ...
    _error = MCP23S17_OK;
    if (port == 0)
        return readReg(MCP23S17_GPIO_A);
...
bool MCP23S17::setPolarity8(uint8_t port, uint8_t mask)
{
    ...
    if (port == 0)
        writeReg(MCP23S17_POL_A, mask);
    if (port == 1)
        writeReg(MCP23S17_POL_B, mask);
    if (_error != MCP23S17_OK)
    {
        return false;
    }
...
bool MCP23S17::setPullup8(uint8_t port, uint8_t mask)
{
    ...
    if (port == 0)
        writeReg(MCP23S17_PUR_A, mask);
    if (port == 1)
        writeReg(MCP23S17_PUR_B, mask);
    if (_error != MCP23S17_OK)
    {
        return false;
    }
